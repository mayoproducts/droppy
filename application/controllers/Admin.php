    ...
    function ajax() {
        ...
        if($page == 'uploads') {
            ...
            foreach ($all_data as $key => $row) {
                $row['#'] = ($row['Status'] == 'ready' ? '<a href="#" onclick="deleteUpload(\'' . $this->config->config['site_url'] . 'admin/uploads/page/' . $page . '/delete/' . $row['ID'] . '\'); return false;"><i class="fa fa-trash"></i> Destroy</a>' : '');
